    ...
    <div className="wds-container page">
      ...
      <div className={hasSent ? "hidden" : ""}>
        <form onSubmit={handleSubmit(handleFormSubmit)}>
          ...
          <button className="send-btn disabled:opacity-30 disabled:cursor-progress" disabled={loading}>Send</button>
        </form>
      </div>
      <div className={`${hasSent ? "" : "hidden"} text-center`}>
          <h2 className="text-6xl font-bold mb-7">Sent</h2>
          <p className="font-semibold mb-10">Thank you for your message. I will be in touch very soon!</p>
          <div className="mt-10">
            <Link href="/" className="text-xl text-secondary py-3 px-5 bg-white rounded hover:bg-secondary hover:text-white transition-colors">
              Back to homepage
            </Link>
          </div>
        </div>
